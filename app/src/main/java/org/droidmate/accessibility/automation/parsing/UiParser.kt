...
import org.droidmate.accessibility.automation.extensions.BitmapProcessor
import org.droidmate.accessibility.automation.extensions.getBounds
import org.droidmate.accessibility.automation.extensions.markedAsOccupied
import org.droidmate.accessibility.automation.extensions.toRectangle
import org.droidmate.accessibility.automation.extensions.visibleAxis
import org.droidmate.accessibility.automation.extensions.visibleAxisR
...
import org.droidmate.accessibility.automation.utils.safeCharSeqToString
...
abstract class UiParser {
    ...
    companion object {
        ...
        private fun computeIdHash(xPath: String, window: DisplayedWindow) =
            computeIdHash(xPath, window.layer)
    }

    protected suspend fun createBottomUp(
        w: DisplayedWindow,
        node: AccessibilityNodeInfo,
        index: Int = 0,
        parentXpath: String,
        nodes: MutableList<UiElementProperties>,
        img: Bitmap?,
        parentH: Int = 0
    ): UiElementProperties? {
        ...
        if (!refresh) {
            Log.d(
                TAG,
                "refresh node failed, this will lead to illegal state exception later-on, " +
        ...
        val children: List<UiElementProperties?> = (nChildren - 1 downTo 0)
            ...
            .map { (i, childNode) ->
                // bottom-up strategy, process children first (in drawingOrder) if they exist
                if (childNode == null) {
                    debugOut("ERROR child nodes should never be null")
                }
                createBottomUp(w, childNode, i, "$xPath/", nodes, img, parentH = idHash)
                    // recycle children as we requested instances via getChild which have to be released
    ...
    private fun AccessibilityNodeInfo.createWidget(
        w: DisplayedWindow,
        xPath: String,
        children: List<UiElementProperties>,
        img: Bitmap?,
        idHash: Int,
        parentH: Int,
        processedNodes: List<UiElementProperties>
    ): UiElementProperties {
        val nodeRect = Rect()
        ...
        props.add("labeledBy = ${this.labeledBy}")
        ...
        val visibleAreas = if (!isEnabled || !isVisibleToUser) {
            emptyList()
        } else {
            nodeRect.visibleAxis(
                w.area,
                isSingleElement = true
            ).map { it.toRectangle() }.let { area ->
                if (area.isEmpty()) {
                    val childrenC =
                        children.flatMap { boundsList -> boundsList.visibleAreas } // allow the parent boundaries to contain all definedAsVisible child coordinates
                    uncoveredArea = false
...
// 	}
        ...
        val visibleBounds: Rectangle = when {
            // no uncovered area means this node cannot be visible
            visibleAreas.isEmpty() -> Rectangle(
                0,
                0,
                0,
                0
            ...
            else -> with(mutableListOf<Rectangle>()) {
                addAll(visibleAreas)
...
// 					),
            ...
            hasClickableDescendant = children.any(isClickableDescendant).let { hasClickableDescendant ->
                ...
                if (!hasClickableDescendant && selected.isEnabled()) {
                    ...
                    processedNodes.any {
                        visibleBounds.contains(it.visibleBounds) && isClickableDescendant(
                            it
                        )
                    }
    ...
    protected val nodeDumper: (serializer: XmlSerializer, width: Int, height: Int) -> NodeProcessor =
        { ser: XmlSerializer, width: Int, height: Int ->
            { node: AccessibilityNodeInfo, index: Int, _ ->
                val nodeRect = Rect()
                // determine the 'overall' boundaries these may be outside of the
                // app window or even outside of the screen
                ...
                ser.addAttribute(
                    "isInputField",
                    node.isEditable
                ) // could be useful for custom widget classes to identify input fields
