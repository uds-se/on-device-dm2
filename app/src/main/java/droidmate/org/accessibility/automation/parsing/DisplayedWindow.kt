    ...
    companion object {
        operator fun invoke(
            ...
            val root: AccessibilityNodeInfo? =
                deviceRoot
                    ?: wInfo.root // REMARK: do not recycle root nodes, only instances requested via getChild
            ...
            debugOut(
                "start window visibility computation for ${root?.packageName} $outRect" +
                        ...
                        if (api >= 24) "title='${wInfo.title}' " else "" +
                                "desc='${root?.contentDescription}', " +
                                "accF=${wInfo.isAccessibilityFocused}, " +
                                "isF=${wInfo.isFocused}, " +
                                "${wInfo.layer}, " +
                                "isActive=${wInfo.isActive}",
                debug
            )
