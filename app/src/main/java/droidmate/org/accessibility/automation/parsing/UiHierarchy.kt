...
//			debugOut("windows to extract: ${windows.map { "${it.isExtracted()}-${it.w.pkgName}:${it.w.windowId}[${visibleOuterBounds(it.area)}]" }}")
			...
			windows.forEach { w: DisplayedWindow ->
				...
				if (w.isExtracted() && !w.isLauncher) {
					...
					Log.d(
						TAG, "${w.w.pkgName}:${w.w.windowId} ${visibleOuterBounds(w.initialArea)} " +
	...
	suspend fun getXml(engine: IWindowEngine): String = debugT(" fetching gui Dump ", {
		...
		StringWriter().use { out ->
			...
			engine.exec(
				nodeDumper(ser, engine.lastDisplayDimension.width, engine.lastDisplayDimension.height)
			) {
				ser.endTag("", "node")
			}
	 ...
	 * **/
	suspend fun any(engine: IWindowEngine, retry: Boolean = false, cond: SelectorCondition): Boolean {
...
//			Log.d(TAG, "roots are ${roots.map { it.packageName }}")
	 ...
	 * */
	@JvmOverloads
	suspend fun waitFor(env: AutomationEngine, timeout: Long = 10000, cond: SelectorCondition): Boolean {
	 ...
	 * */
	suspend fun waitFor(env: AutomationEngine, timeout: Long, pollTime: Long, cond: SelectorCondition): Boolean =
		coroutineScope {
